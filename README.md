...
- история тренировок между запусками: вкладка «Прогресс» с точностью по дням, средним и лучшим результатом и сложными символами за всё время (на Windows; на телефоне такой вкладки нет — там записанное на бумаге сверяют с текстом задания);
...
- телефон устроен проще: пять вкладок — «Тренировка», «Обучение», «На слух», «Передача», «Настройки»; на «Тренировке» группы записывают на бумаге и сверяют с текстом задания («Показать»), ввод ответа в приложении — по кнопке «Проверить вводом ▾» (для экзамена и шагов курса она раскрывается сама); на «Обучении» набор символов выбирается кнопками («Русские буквы», «Латинские буквы», «Русские и латинские», «Цифры»), поиск — по 🔍; «На слух» — отдельная вкладка: звучит символ, из четырёх крупных кнопок выбирается прозвучавший, «Ещё раз» повторяет сигнал;
...
- горячие клавиши Windows: F5 — слушать, Esc — стоп, Ctrl+G — новое задание, Ctrl+E — экзамен, Ctrl+K — вкладка «Передача»;
